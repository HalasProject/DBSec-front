...
export class InventoryService
{
     ...
     * Get modules
     */
    getModules(a,b,c,d,e): Observable<any>
    {
        return this._httpClient.get<InventoryModule[]>(`${env.apiUrl}/modules`).pipe(
            tap((response) => {
    ...
    createModule(): Observable<InventoryModule>
    {
        return this.modules$.pipe(
            take(1),
            switchMap(modules => this._httpClient.post<InventoryModule>(`${env.apiUrl}/module`, {}).pipe(
                map((newModule) => {
    ...
    updateModule(id: string, module): Observable<any>
    {
        return this.modules$.pipe(
            take(1),
            switchMap(modules => this._httpClient.put<InventoryModule>(`${env.apiUrl}/module/${id}`, {
                ...{data:module}
            }).pipe(
                map((updatedModule) => {
    ...
    deleteModule(id: string): Observable<boolean>
    {
        return this.modules$.pipe(
            take(1),
            switchMap(modules => this._httpClient.delete(`${env.apiUrl}/module/${id}`).pipe(
